...
namespace OccaSoftware.Altos.Runtime
{
    internal class VolumetricCloudsRenderPass : ScriptableRenderPass
    {
        ...
        #region Input vars
        private const string profilerTag = "[Altos] VolumetricCloudRenderPass";
        private const string volumeticCloudId = "_VolumetricCloudTexture";
        #endregion
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<PassData>(profilerTag, out var passData))
            {
                ...
                RenderTextureDescriptor rtDescriptor = ConfigurePass(cameraData.cameraTargetDescriptor);
                ...
                passData.SourceDescriptor = rtDescriptor;
        ...
        private void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            ExecutePass(cmd, data.Source, data.SourceDescriptor, data.WorldSpaceCameraPos, data.Camera);
        }
#endif
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            ExecutePass(cmd, cameraData.renderer.cameraColorTargetHandle, cameraData.cameraTargetDescriptor, cameraData.worldSpaceCameraPos, cameraData.camera);
        ...
        private void SetupTargets(RenderTextureDescriptor cameraDescriptor)
        {
            ...
            RenderingUtilsHelper.ReAllocateIfNeeded(ref cloudTarget, cloudDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: cloudId);
            ...
            {
                RenderingUtilsHelper.ReAllocateIfNeeded(ref temporalTarget, cameraDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: taaId);
            }

            RenderingUtilsHelper.ReAllocateIfNeeded(ref mergeTarget, cameraDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: mergeId);
        }
        ...
        public void ExecutePass(CommandBuffer cmd, RTHandle source, RenderTextureDescriptor sourceDescriptor, Vector3 worldSpaceCameraPos, Camera camera)
        {
            CloudShaderParamHandler.SetCloudMaterialSettings(cmd, skyDirector.cloudDefinition);

            TimeManager.Update();
            this.source = source;
            cameraPosition = worldSpaceCameraPos;

            TemporalAA.ProjectionMatrices matrices = taa.SetupMatrices(camera);
            ...
            {
                int width = sourceDescriptor.width;
                int height = sourceDescriptor.height;
                ...
                isTemporalDataValid = taa.IsTemporalDataValid(camera, width, height);

                if (!isTemporalDataValid)
                {
                    frameId = 0;
                    taa.SetupTemporalData(camera, sourceDescriptor, width, height);
            ...
            if (skyDirector.cloudDefinition.useReprojection)
            {
                activeTarget = Reproject(cmd, camera, activeTarget);
            }
            ...
            if (skyDirector.cloudDefinition.useTemporalDenoising)
            {
                activeTarget = TemporalAntiAliasing(cmd, camera, activeTarget);
            }
